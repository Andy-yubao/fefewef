...
Iteration 27 changed only the optical scheduling radius from 30 to 35 m. It cleared the paired random 100 and 300 at means 8027.42 and 8095.16 s, with random-300 P95/maximum 9093.64/9890.49 s. The paired all-directional and all-omni 100-case means were 8881.02 and 7259.03 s, again with zero failures, and all three regressions passed. The speed gain over 30 m is small (27.88 s on random 300), so a fresh OS-entropy 1000-case run was started before promotion.
...
After the controlled iterations, `experiments/t4_analysis/random_benchmark.py` selected 1000 unique 32-bit seeds from operating-system entropy without a preset RNG seed. The full seed manifest was written before execution. `replacement_aware_clear_probe / 731 / 400 / 2` cleared 1000/1000 cases. Virtual time had minimum 5527.82 s, mean 8909.21 s, population standard deviation 611.80 s, median 8908.40 s, P90 9622.82 s, P95 9967.15 s, P99 10467.40 s, and maximum 11133.29 s. The fastest seed was 3581489261 and the slowest was 3266971321. Artifacts are under `experiments/t4_analysis/outputs/random_large_1000_replacement_aware/`.

## Why a mixed-distribution 8000 s result is not claimed

Safe full coverage has a substantial route-length floor and absent channels still require repeated negative measurements. Iteration 22 lowered the reliable random mixed mean to 8353.66 s, not below 8000 s. The IDA*-inspired iteration reached 8121.88 s on random 300 but failed an all-directional case, so it cannot replace the reliable figure. All-omni results below 8000 s are distribution-specific and cannot be presented as mixed or official results.
