...
| $D(A)=\max_{p,q\in A}\lVert p-q\rVert$ | 问题 1 的区域直径 |
...
$$
W(S,\theta;r_-,r_+)=\{g:\ r_-<\lVert g-S\rVert\le r_+,
|\operatorname{adiff}(\beta(S,g),\theta)|\le\delta\}.
$$

**[推导]** 因为第一次获得了正常示向而非 `near` 或 `no_signal`，必有
$5<\lVert G-S_1\rVert\le R$。个体的 $R$ 未知时，用题面上界形成保证包含真值的
保守区域：
...
$$
\mathcal C_f^{\mathrm{theory}}=\{s\in\mathbb R^2:
...
$\mathcal C_f=\mathcal C_f^{\mathrm{theory}}\cap\Omega$。圆外大基线是否带来净收益
留作扩展敏感性分析。
...
$$
p_{\mathrm{det}}(s)=P(\lVert G-s\rVert\le R\mid Y_1),\qquad
...
$$
\mathcal C_r=\{s\in\mathcal C_f:p_{\mathrm{det}}(s)\ge0.25,
q(s)\ge0.12\},
$$
...
$$
U_{\rm geo}(s)=E\left[
\mathbf{1}_{\{\lVert G-s\rVert\le R\}}
\frac{|\sin\alpha|}{\sqrt{\lVert G-S_1\rVert\,\lVert G-s\rVert}}
\right]
$$
...
令 $J=\sum_i h_i^{\mathsf T}h_i/\sigma^2$，定义
...
$$
S_2^*=\arg\min_s \widehat{\mathbb E}[D(\mathcal F_2(s,Y_2))].
$$
...
3. 若机器狗总任务时间更重要，联合比较 expected-diameter、geometry 和 E-optimal
   三个最终点，最小化 $D+\lambda\lVert S_2-S_1\rVert/5$，由问题 3 的时间价值确定
   $\lambda$；
