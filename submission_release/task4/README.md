...
2026-09-12 新增独立策略 `adaptive_double_ring_clear_probe`：在双环25点覆盖上加入50 m有界光学覆盖、清除点最多两频道补测，以及发现满16源后的可靠收尾。参数锁定后的本地验证共1000例均全清；其中混合300例均值6641.99→6377.34 s，固定16源200例6337.99→5742.93 s、6000 s内全清率36%→62.5%。全定向300例均值7056.22 s，尚未达到6000 s。它已注册，运行时显式使用 `--strategy adaptive_double_ring_clear_probe`。完整配对、分层、退化案例和复现命令见[第二轮双环实验报告](../experiments/t4_analysis/outputs/toward6000_v2/REPORT.md)。
...
- `adaptive_double_ring_clear_probe`：双环进一步优化；50 m中心加十环点光学覆盖，最多两个active频道清除点补测，16源已发现后停止unseen检测并转可靠收尾；保持25点发现证书，拒绝覆盖点替代。新增验证1000例全清，但整体混合均值仍高于6000 s，且存在逐例变慢。
