...
通过 Windows 到 WSL 的 TCP 转发，程序已连接官方模拟器并完成 1 局问题 3 演练。运行入口为 `task3/experiments/run_practice.py`。演练指标为
...
| 案例编码 | 真实目标数 | 清除数 | 清除比例 | 总虚拟时间 (s) | 平均定位清除时间 (s/个) | 程序运行时间 (s) | 动作数 | 覆盖点 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| J2TJ-2H73-YG5X-Y4AZ | 16* | 16 | 1.000 | 5903.926 | 368.995 | 2.910 | 217 | 7/7 |

星号表示接口成功清除 16 个并以 `upper_bound_reached` 正常结束；由于题面规定目标数至多为 16，清除比例可严格确定为 100%，同时等待 GUI 显示的真实总数作交叉核对。该局共记录 219 个接受响应（含 `/enter` 和 `/exit`），网络重试为 0；五类虚拟时间分解为移动 4680.926 s、切频 164 s、检测 940 s、光学定位 87 s、激光清除 32 s，恰好合计 5903.926 s。

原始脱敏日志为 `task3/results/raw/practice/J2TJ-2H73-YG5X-Y4AZ-20260911-132112.jsonl`，汇总为同名 `.summary.json`，聚合表为 `task3/results/tables/practice_summary.csv`。单局只验证了接口兼容和一次完整清除，不能代表总体性能；仍建议扩充至至少 100 局并覆盖少于 16 个源、MEC 认证清除、兜底多次失败及网络异常等情况。本节只报告实际取得的数据，不把离线 mock 伪装成官方演练。
