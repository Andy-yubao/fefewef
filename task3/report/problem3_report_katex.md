...
通过 Windows 到 WSL 的 TCP 转发，程序已连接官方模拟器并完成 2 局问题 3 演练。运行入口为 `task3/experiments/run_practice.py`。演练指标为
...
首局 GUI 已交叉确认真实目标数为 16。第二局程序清除 12 个目标、完成全部七点必接收覆盖并把其余 8 个频道认证为 `absent`，因此在题设和接口正常前提下总数为 12；用户同时确认模拟器正常完成该局。首局五类虚拟时间分解为移动 4680.926 s、切频 164 s、检测 940 s、光学定位 87 s、激光清除 32 s；第二局依次为 3205.436 s、157 s、865 s、36 s、24 s。两局分别记录 219 和 187 个接受响应（均含 `/enter` 和 `/exit`），网络重试均为 0。

原始脱敏日志位于 `task3/results/raw/practice/`，文件名前缀分别为上述案例编码，汇总为同名 `.summary.json`，聚合表为 `task3/results/tables/practice_summary.csv`。Geometry 局平均定位清除时间较 B3 局低 11.709 s/个，移动时间也由 292.558 s/个降至 267.120 s/个；但两局源数、位置和误差不同，这只是后续重复试验的积极线索，不能当作同案例配对效应。仍建议扩充样本并覆盖 MEC 认证清除、兜底多次失败及网络异常等情况。本节只报告实际取得的数据，不把离线 mock 伪装成官方演练。
