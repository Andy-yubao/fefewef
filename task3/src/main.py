...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--mode", choices=["two_stage", "enroute", "rolling_hard", "hybrid"],
                   default="hybrid")
    p.add_argument("--local-family", choices=["geometry", "e_optimal", "expected_diameter", "shortlist"],
                   default="shortlist")
...
def main(argv: list[str] | None = None) -> int:
    ...
    planner = PlannerConfig(local_action_limit=args.local_action_limit, seed=args.seed)
    client = SimulatorClient(client_cfg, args.log)
    controller = SearchController(client, args.mode, args.local_family, PhysicalConfig(), planner, args.known_total)
    try:
        ...
        args.summary.write_text(json.dumps({"success": False, "error": repr(exc),
